...
![Milestone 4: Gerstner waves with crest foam, and a buoyant crate riding the swell](CFD_But_Actually_Fun.png)

*Milestone 4 stopping point: a four-component Gerstner wave field with analytic
normals and crest foam, plus a crate floating by sampled Archimedes buoyancy
with full 3D rigid-body rotation. The darker band at its waterline is the
"wet wood" shading.*
...
| 4 — Gerstner waves + buoyant rigid body, fixed 120 Hz timestep | ✅ **You are here** |
| 5 — Wake field: moving bodies leave ripples | ⏳ Next |
| 6 — Procedural jetski + rider (two-bone IK, wave absorption) | ⏳ Planned |
...
- **Renderer:** hand-rolled DirectX 12 — device, swap chain, RTV/DSV heaps,
  fence handshake, three PSOs (solid water, wireframe water, crate), and a
...
| File | Role |
|---|---|
| `main.cpp` | The whole C++ side: window, D3D12 bootstrap, wave field, rigid body, frame loop |
| `water_vs.hlsl` | Vertex shaders: `main` (world-space water), `mainModel` (body-space crate) |
| `water_ps.hlsl` | Pixel shaders: `main` (water, foam + glint), `psCrate` (wet-band wood) |
| `CFD_But_Actually_Fun.png` | The Milestone 4 screenshot shown above |

## Physics notes (the actual point of the project)

- **Dispersion is real.** Every component obeys ω = √(g·k), so long swells
  visibly outrun short chop. Not an art choice — four lines of math.
- **Gerstner is parametric.** Vertices move horizontally *and* vertically,
  which buys sharp crests and flat troughs. Querying height at a world point
  means inverting that horizontal shift; three fixed-point iterations do it.
- **Buoyancy is sampled, not integrated.** Points on the hull underside push
  up in proportion to submersion; tilt falls out of the lever arms. This is
  why the crate leans into wave faces instead of bobbing flat.
- **Fixed timestep is a promise.** Physics ticks at 120 Hz regardless of
  display refresh, so the crate falls identically on every machine.

## Roadmap

Next: a wake field so moving bodies leave ripples in their path. Then the
procedural jetski + rider module — hands IK-locked to handlebars, hips
